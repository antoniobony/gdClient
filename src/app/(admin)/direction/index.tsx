...
const Directions = ({name}:Props) =>{ 
    ...
    const [directionModif,setDirectionModif]= useState({designation:"",code:""})
    const [direction,setDirection]= useState({designation:"",code:""})
    ...
    const handleSubmit=(e: { preventDefault: () => void; })=>{
        e.preventDefault();
        directionModif.designation ? Update():addDate()
        handleClose()
    ...
    const handleClose=()=>{
        setDirectionModif({designation:"",code:""})
        setDirection({designation:"",code:""})
        setOpen(false)
    }
    ...
    const addDate = ()=>{
        axios.post(
            process.env.NEXT_PUBLIC_API_BASE_URL+"/direction",
        {
    ...
    const Update =()=>{
        ...
        const id = da.find((d => d.designation  === directionModif.designation))
        let ids = id?.id
        axios.put(
            process.env.NEXT_PUBLIC_API_BASE_URL+`/direction/${ids}`,
        {
            "designation":direction.designation ?direction.designation:directionModif.designation,
            "code":direction.code ? direction.code:directionModif.code
        },{ 
            headers:{
            "Accept":"*/*",
            Authorization:"Bearer "+Cookies.get("token"),
            ...
            <div className="pt-5">
                <Header name={name} button={    
                    ...
                    <form className="mt-4 space-y-6" onSubmit={handleSubmit}>
                        <Field type="text" placeholder="designation" name="designation" className={inputStyles} onChange={Change} defaultValue={directionModif.designation?directionModif.designation:""}/> 
                        <Field type="text" placeholder="code" name="code" className={inputStyles} onChange={Change} defaultValue={directionModif.code?directionModif.code:""}/> 
                        <button type="submit" className={`focus-offset-2 mt-4 flex w-full justify-center rounded-md border border-transparent 
                            ...
                            `}>
                               {directionModif.code ? "Modifier":"Ajouter"}
                            </button>
                    </form>
                </MyModal>}
                 isSmallText/>
