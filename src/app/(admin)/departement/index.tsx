...
const Departements = ({name}:Props) =>{ 
    ...
    const [departementModif,setDepartementModif]= useState({designation:"",code:""})
    const [departement,setDepartement]= useState({designation:"",code:""})
    ...
    const handleSubmit=(e: { preventDefault: () => void; })=>{
        e.preventDefault();
        departementModif.designation ? Update():addDate()
        handleClose()
    ...
    const handleClose=()=>{
        setDepartementModif({designation:"",code:""})
        setDepartement({designation:"",code:""})
        setOpen(false)
    }
    ...
    const addDate = ()=>{
        axios.post(
            process.env.NEXT_PUBLIC_API_BASE_URL+"/departement",
        {
    ...
    const Update =()=>{
        ...
        const id = da.find((d => d.designation  === departementModif.designation))
        let ids = id?.id
        axios.put(
            process.env.NEXT_PUBLIC_API_BASE_URL+`/departement/${ids}`,
        {
            "designation":departement.designation ?departement.designation:departementModif.designation,
            "code":departement.code ? departement.code:departementModif.code
        },{ 
            headers:{
            Authorization:"Bearer "+Cookies.get("token"),
          }}).then(
        ...
        const RemoveDepartement=(id:string)=>{
        
            axios.delete(process.env.NEXT_PUBLIC_API_BASE_URL+`/departement/${id}`,{
              headers:{
              Authorization:"Bearer "+Cookies.get("token"),
            }}).then(   
              r=>{
            ...
            <div className="pt-5">
                <Header name={name} button={    
                    ...
                    <form className="mt-4 space-y-6" onSubmit={handleSubmit}>
                        <Field type="text" placeholder="designation" name="designation" className={inputStyles} onChange={Change} defaultValue={departementModif.designation?departementModif.designation:""}/> 
                        <Field type="text" placeholder="code" name="code" className={inputStyles} onChange={Change} defaultValue={departementModif.code?departementModif.code:""}/> 
                        <button type="submit" className={`focus-offset-2 mt-4 flex w-full justify-center rounded-md border border-transparent 
                            ...
                            `}>
                               {departementModif.code ? "Modifier":"Ajouter"}
                            </button>
                    </form>
                </MyModal>}
                 isSmallText/>
